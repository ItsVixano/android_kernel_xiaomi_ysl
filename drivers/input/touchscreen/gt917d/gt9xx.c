...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	if(ret < 0)
	{
		...
		//pinctrl_select_state(ts->ts_pinctrl, ts->erst_as_default);
...
static int gtp_power_switch(struct i2c_client *client, int on)
{
	...
	if (on) {
	    //add by likang for modify sequence diagram @20171130
	    struct goodix_ts_data *ts = i2c_get_clientdata(client);
...
//begin songxihao 20171019 for gesture
#if GTP_GESTURE_WAKEUP
struct kobject *goodix_gesture_kobj;
...
//add by likang for sequence diagram @20171130
